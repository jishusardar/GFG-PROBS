...
class Solution {
  public:
    ...
    void inorderpush(Node* root,vector<int>&ans,int &i)
    {
        if(!root)
        return;
        inorderpush(root->left,ans,i);
        root->data=ans[i];
        i++;
        inorderpush(root->right,ans,i);
    }
    void correctBST(Node* root) {
        ...
        if(second==-1){
            swap(ans[first],ans[first+1]);
        }
        else{
            swap(ans[first],ans[second]);
        }
        int i=0;
        inorderpush(root,ans,i);
    }
};
