...
class Solution {
  public:
    ...
    Node* CreateBST(int pre[],int &index,int start,int end,int N)
    {
        if(index==N||pre[index]<start||pre[index]>end)
        return NULL;
        Node* root=newNode(pre[index++]);
        root->left=CreateBST(pre,index,start,root->data,N);
        root->right=CreateBST(pre,index,root->data,end,N);
        return root;
    }
    Node* Bst(int pre[], int size) {
        // code here
        int index=0;
        return CreateBST(pre,index,INT_MIN,INT_MAX,size);
    }
};
